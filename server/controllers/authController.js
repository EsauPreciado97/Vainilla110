  ...
  bcrypt
    ...
    .then((savedUser) => {
      transporter.sendMail({
        to: email,
        from: "esauplm@gmail.com",
        subject: "Verifica tu cuenta en Vainilla 110 Marketplace",
        html: `
...
exports.verifyAccount = (req, res, next) => {
  ...
  Account.findOne({
    ...
    .then((account) => {
      if (!account) {
        const error = new Error(
          "Token in the url is tempered, validation failed"
        );
  ...
  bcrypt
    ...
    .then((savedSeller) => {
      transporter.sendMail({
        to: email,
        from: "esauplm@gmail.com",
        subject: "Verify your Account on Vainilla 110",
        html: `
                      <p>Please verify your email by clicking on the link below - FoodHub</p>
                      <p>Click this <a href="http://localhost:3002/auth/verify/${token}">link</a> to verify your account.</p>
                    `,
