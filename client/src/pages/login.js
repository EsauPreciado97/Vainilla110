...
export default function Login() {
  ...
  if (errors) {
    if (errors.includes("Usuario / Contraseña incorrectos"))
      incorrectCredentialsError = errors;
    if (errors.includes("Verifica tu cuenta para continuar")) verifyEmailError = errors;
  }
        ...
        <form noValidate onSubmit={handleSubmit}>
          {signUpSuccess && (
            <Typography variant="body2" className={classes.customSuccess}>
              Tu cuenta ha sido registrada exitosamente, para poder ingresar
              verifica tu usuario desde el correo electronico que te enviamos
            </Typography>
          )}
          <TextField
            id="email"
          ...
          <small className={classes.small}>
            Aun no tienes una cuenta ? Registrate <Link to="/register">aquí</Link>
          </small>
        </form>
      </Grid>
      <Grid item sm />
